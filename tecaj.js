...
/* Callback function called on request.
 Checks if any 'checked' error has occurred,
...
db.findOneOf([{'key' : 'display', 'value' : '08102013'}], models.CurrencyTable, function(result) {
    if (!result) {
        console.log('requesting from web');
        request(link, callback);
    } else {
        console.log('it exists in the database: ' + result);
    }
});
